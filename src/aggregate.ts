 ...
 */
function parseLineTime(line: string, from: number): number {
  let i = line.charCodeAt(from) === 0x2c ? from + 1 : -1 // ','
  if (i < 0 || line.charCodeAt(i) !== 0x22) return NaN // '"'
  if (line.startsWith('"time":', i)) {
    ...
    if (i === d0 || !line.startsWith(',"time":', i)) return NaN
    i += 8
  } else {
    return NaN
  }
  const n0 = i
  if (line.charCodeAt(i) === 0x2d) i++ // '-'
  while (i < line.length) {
    const c = line.charCodeAt(i)
    if (c < 0x30 || c > 0x39) break
    i++
...
function skipJsonValue(line: string, j: number): number {
  ...
  if (c === 0x7b || c === 0x5b) {
    ...
    while (k < line.length) {
      ...
      if (ch === 0x22) { inStr = true; k++; continue }
      if (ch === 0x7b || ch === 0x5b) d++
      else if (ch === 0x7d || ch === 0x5d) { d--; if (d === 0) return k + 1 }
      k++
    }
    return -1
  }
  ...
  while (k < line.length) {
    const ch = line.charCodeAt(k)
    if (ch === 0x2c || ch === 0x7d || ch === 0x5d || ch <= 0x20) break
    k++
  }
  return k
}
...
function readAssistantData(line: string): { turn: number; step: number; usage: UsageSample } | null {
  ...
  let esc = false
  let strStart = -1
  let curKey: string | null = null // 刚读完的键名，等待其值
  let curKeyDepth = -1
  ...
  while (i < line.length) {
    ...
    if (awaitingValue) {
      ...
      if (wanted && curKey === 'usage' && c === 0x7b) {
        ...
        awaitingValue = false
        curKey = null
        if (turn !== null && step !== null) break // 三个目标齐了：不再扫行尾的 surfaceOp
        i = e
        continue
      }
    ...
    if (c === 0x22) { inStr = true; strStart = i; i++; continue }
    if (c === 0x7b || c === 0x5b) {
      depth++
      if (curKey === 'data' && curKeyDepth === depth - 1) dataDepth = depth
      curKey = null
      i++
      continue
    }
    if (c === 0x7d || c === 0x5d) {
      if (depth === dataDepth) dataDepth = -1 // data 对象已闭合
      depth--
      curKey = null
      i++
      continue
    }
    if (c === 0x3a) { awaitingValue = true; i++; continue }
    if (c === 0x2c) { curKey = null; i++; continue }
    i++
  }

  if (usageStart < 0 || turn === null || step === null) return null
  let usage: unknown
  try {
...
function foldLine(st: FoldState, line: string): void {
  ...
  if (ev !== null) {
    ...
    const ft = parseLineTime(line, body)
    if (ft === ft) t = ft // NaN 自比较为 false
  }
  if (t === 0 || t !== t) {
    // 快路径未命中或 time 非数字：回退全量解析取权威值
    try {
      const ev2: any = JSON.parse(line)
  ...
  if (ty === 'turn/start') {
    if (typeof ev?.data?.turn === 'number') st.turnSet.add(ev.data.turn)
    return
  }
  if (ty === 'step/end') {
    out.steps += 1
  ...
  if (ty === 'llm/retry-started') {
    const last = st.last
    if (last && last.turn === ev?.data?.turn && last.step === ev?.data?.step) st.last = null
    return
  }
...
function computeMetrics(
...
): { metrics: UsageMetrics; dayTotals: Record<string, number> } {
  ...
  let turns = 0
  let steps = 0
  for (const s of logEntries) { turns += s.turns; steps += s.steps }
  ...
  let deletedTokens = 0
  const countEntry = (id: string, days: DayBuckets, isArchived: boolean): void => {
    if (subagentBare.has(bareId(id))) {
      subagentSessions++
      for (const b of Object.values(days)) {
        const t = b[0] + b[1] + b[2] + b[3]
        subagentTokens += t
        if (isArchived) deletedTokens += t
      }
    } else {
      mainSessions++
      if (isArchived) for (const b of Object.values(days)) deletedTokens += b[0] + b[1] + b[2] + b[3]
    }
  }
  for (const s of Object.values(sessions)) countEntry(s.id, s.days, false)
  for (const s of Object.values(archived)) countEntry(s.id, s.days, true)
  for (const o of Object.values(orphans)) countEntry(o.id, { [o.day]: o.b }, false)
