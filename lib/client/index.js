...
function HeatmapCard(props) {
    ...
    while (lastMon > 0 && months[lastMon] === '')
        lastMon--;
    ...
    var monthsRow = showMonths
        ? h('div', { className: 'uh-months' }, 
        // 首列占位：月份轴与格子共用 --uh-tracks，缺了这个槽位标签就会整体左移一轨
        h('span', { className: 'uh-monthPad', key: '__pad' }), months.map(function (mm, i) {
    ...
    var calStyle = { '--uh-cols': range.columns.length, '--uh-gutter': gutter + 'px', '--uh-cell-min': effCellMin + 'px' };
    // 当前模式的取值集合 → 分位档
    ...
    if (!hasData) {
        ...
        body = h('div', { className: 'uh-calWrap', ref: wrapRef }, h('div', { className: 'uh-cal', style: calStyle }, h('div', { className: 'uh-main' }, monthsRow, h('div', { className: 'uh-weekCells' }, 
        // 周视图无行标，但保留同一首列轨道 → 三种模式左缘一致，切换时不横跳
        h('span', { className: 'uh-weekPad', key: '__pad' }), weekCells))));
    }
    else {
        ...
        body = h('div', { className: 'uh-calWrap', ref: wrapRef }, h('div', { className: 'uh-cal', style: calStyle }, h('div', { className: 'uh-main' }, monthsRow, h('div', { className: 'uh-cells' }, 
...
function UsageHeatmapPanel() {
    ...
    var kids = [
        h('div', { className: 'uh-header', key: 'header' }, h('div', { className: 'uh-brand' }, h('strong', { className: 'uh-brandName' }, SECTION_LABEL), h('p', { className: 'uh-brandDesc' }, '本地 DSH 会话日志聚合 · 累计 Token、活跃热力图与活动洞察')), h('div', { className: 'uh-headerRight' }, h('span', { className: 'uh-sum' }, headerSum), h(Btn, { kind: 'primary', disabled: s.loading, onClick: function () { s.load(true); } }, s.loading ? '刷新中…' : '刷新'))),
    ];
    if (s.err) {
...
function installSettingsNavIcon() {
    ...
    function sync() {
        ...
        if (!wanted)
            return; // 文本未就绪：什么都不标记，绝不因空串而认领整列
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (String(row.textContent || '').trim() === wanted)
                ...
                row.removeAttribute(NAV_ICON_MARKER); // 认领是幂等的：别行的陈旧标记一并清掉
        }
    }
    function schedule() {
